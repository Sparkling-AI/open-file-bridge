...
def check_request(headers) -> tuple[bool, int | None, str]:
    ...
    mode = security_mode()
    if mode == "UNLOCKED":
        return False, 503, ("bridge unlocked — open the Open File Bridge settings page "
    ...
    def __init__(self, pattern, rel):
        ...
        if pattern in DEFAULT_IGNORE:
            ...
            msg = (f"'{rel}' is excluded by ignore settings (pattern: {pattern}); "
                   f"tell the user — ignore patterns are editable in the "
                   f"Open File Bridge settings page (the local picker)")
        super().__init__(msg)
...
def resolve_any(rel: str):
    ...
    ros = enabled_roots()
    if not ros:
        raise PermissionError("no shared folder configured — open the Open File Bridge "
                              "settings page")
    rel = (rel or "").strip()
...
class Handler(http.server.BaseHTTPRequestHandler):
        ...
        try:
                ...
                if not _state_load().get("allow_reveal"):
                    return self._json(403, {
                        "error": "reveal is disabled on this machine — ask the "
                                 "user to enable it in the Open File Bridge settings "
            ...
            return self._json(404, {"error": str(e), "excluded": True,
                                    "hint": "excluded by settings — tell the user; "
                                            ...
                                            "Open File Bridge settings page"})
    ...
    PICKER_HTML = """<!doctype html><html><head><meta charset="utf-8"><title>Open File Bridge</title>
<link id="favlink" rel="icon" type="image/png" href="data:image/png;base64,">
...
<button onclick="stopBridge()" style="background:#fff1f0">Stop Open File Bridge</button>
...
<script>
document.getElementById('logo').src=document.getElementById('favlink').href;
    ...
    def _install_reopen_delegate(self):
        try:
            ...
            def _imp(_self, _cmd, _sender, _has_vis):
                ...
                fn = self._reopen_fn
                print("Dock icon clicked — opening the Open File Bridge page")
                if fn is not None:
...
def main():
    ...
    if not root:
        ...
        print("App started from its icon — opening the Open File Bridge page...")
